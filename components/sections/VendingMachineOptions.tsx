...
/**
 * VendingMachineOptions component displays a categorized list of 
 * vending machine options that can be selected by the client
 */
const VendingMachineOptions = (): JSX.Element => {
  ...
  return (
    <section className="py-16 bg-white" id="vending-machine-options">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div>
          ...
          <div className="flex p-1 space-x-1 bg-blue-50 rounded-xl mb-8">
            {Object.keys(categories).map((category) => (
              <button
                key={category}
                onClick={() => setActiveCategory(category)}
                className={`w-full py-3 text-sm font-medium leading-5 rounded-lg focus:outline-none focus:ring-2 ring-offset-2 ring-offset-blue-400 ring-blue-500 ring-opacity-60 ${
          ...
          <div>
            {Object.entries(categories).map(([category, machines]) => (
              <div
                key={category}
                className={`bg-white p-3 rounded-xl focus:outline-none ${activeCategory === category ? 'block' : 'hidden'}`}
              >
                <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {machines.map((machine) => (
                    <div
                      key={machine.id}
                      className={`bg-white p-4 rounded-lg border transition-all hover:shadow-lg cursor-pointer ${
                    ...
                    >
                      <div className="aspect-w-4 aspect-h-3 mb-4 bg-gray-100 rounded-md overflow-hidden">
        ...
        {selectedMachine && (
          <div className="mt-12 bg-blue-50 p-6 rounded-xl">
            <div className="grid md:grid-cols-2 gap-8">
              <div>
                <h3 className="text-2xl font-bold text-gray-900 mb-4">{selectedMachine.name}</h3>
                <p className="text-gray-700 mb-6">{selectedMachine.description}</p>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="bg-white p-3 rounded-lg">
                    <h4 className="text-sm font-semibold text-gray-900 mb-1">Dimensions</h4>
                    <p className="text-gray-700">{selectedMachine.dimensions}</p>
                  </div>
                  <div className="bg-white p-3 rounded-lg">
                    <h4 className="text-sm font-semibold text-gray-900 mb-1">Capacity</h4>
                    <p className="text-gray-700">{selectedMachine.capacity}</p>
                  </div>
                </div>
              </div>
                ...
                <div className="mt-4">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {selectedMachine.category === 'snack' && (
                      <>
                        <span className="bg-blue-100 text-blue-800 text-xs font-semibold px-2.5 py-0.5 rounded-full">Break Rooms</span>
                        <span className="bg-blue-100 text-blue-800 text-xs font-semibold px-2.5 py-0.5 rounded-full">Waiting Areas</span>
                        <span className="bg-blue-100 text-blue-800 text-xs font-semibold px-2.5 py-0.5 rounded-full">Employee Lounges</span>
                      </>
                    )}
                    {selectedMachine.category === 'beverage' && (
                      <>
                        <span className="bg-green-100 text-green-800 text-xs font-semibold px-2.5 py-0.5 rounded-full">Cafeterias</span>
                        <span className="bg-green-100 text-green-800 text-xs font-semibold px-2.5 py-0.5 rounded-full">Break Rooms</span>
                        <span className="bg-green-100 text-green-800 text-xs font-semibold px-2.5 py-0.5 rounded-full">High Traffic Areas</span>
                      </>
                    )}
                    {selectedMachine.category === 'combo' && (
                      <>
                        <span className="bg-purple-100 text-purple-800 text-xs font-semibold px-2.5 py-0.5 rounded-full">Driver Lounges</span>
                        <span className="bg-purple-100 text-purple-800 text-xs font-semibold px-2.5 py-0.5 rounded-full">Limited Space Areas</span>
                        <span className="bg-purple-100 text-purple-800 text-xs font-semibold px-2.5 py-0.5 rounded-full">Multi-Purpose Rooms</span>
                      </>
                    )}
                    {selectedMachine.category === 'food' && (
                      <>
                        <span className="bg-yellow-100 text-yellow-800 text-xs font-semibold px-2.5 py-0.5 rounded-full">Cafeterias</span>
                        <span className="bg-yellow-100 text-yellow-800 text-xs font-semibold px-2.5 py-0.5 rounded-full">Lunch Rooms</span>
                        <span className="bg-yellow-100 text-yellow-800 text-xs font-semibold px-2.5 py-0.5 rounded-full">24/7 Facilities</span>
                      </>
                    )}
                  </div>
                </div>
        ...
        <div className="mt-12 bg-gray-50 p-6 rounded-lg">
          <h3 className="text-xl font-bold text-gray-900 mb-4">Custom Options Available</h3>
          <p className="text-gray-700">
            All our vending machines can be customized with additional features to meet your specific needs:
          </p>
          <div className="mt-4 grid md:grid-cols-3 gap-4">
            <div className="bg-white p-4 rounded-lg">
              <h4 className="font-semibold text-gray-900 mb-2">Payment Systems</h4>
