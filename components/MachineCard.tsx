...
export const MachineCard = ({
  ...
  return (
    <motion.article
      ...
      className={`group relative flex flex-col overflow-hidden rounded-2xl transition-all duration-500 ${isActive
    ...
    >
      ...
      >
        ...
        >
          <span
            className={`px-3 py-1.5 rounded-full text-xs font-medium text-white shadow-lg ${machine.category === 'refrigerated' ? 'bg-blue-600' : 'bg-green-600'
              }`}
        ...
        <div className="relative z-0 h-full w-full bg-gradient-to-r from-[#FD5A1E]/5 to-transparent">
          <SEOOptimizedImage
            isAboveFold={false} // Sets loading="lazy"
            src={machine.image}
            alt={`${machine.name} 
            ...
            className={`object-contain p-4 transition-all duration-700 ${isActive ? 'scale-105' : 'scale-100'
      ...
      <div className={`${getCardSpacing()} flex-1 flex flex-col`}>
        ...
        {displayHighlights.length > 0 && (
          <div className="mb-4 flex-1">
            <h3 className={`text-[#F5F5F5] font-bold flex items-center mb-3 ${variant === 'compact' ? 'text-sm' : 'text-base'
              }`}>
              <StarIcon
            ...
            <ul className={`grid gap-2 ${variant === 'related' || variant === 'compact'
                ? 'grid-cols-1'
              ...
              {displayHighlights.map((highlight, idx) => (
                ...
                >
                  <CheckCircleIcon
                    ...
                    aria-hidden="true"
                  />
                  <span className={`text-[#F5F5F5] ${variant === 'compact' ? 'text-xs' : 'text-sm'
                    }`}>
                    {highlight}
                  </span>
                </motion.li>
        ...
        {variant !== 'compact' && (
          <div className="grid grid-cols-1 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-3 mb-6">
            <div className="bg-[#000000]/50 p-3 rounded-lg border border-[#333333]">
              ...
              <div className={`text-[#F5F5F5] font-medium ${variant === 'related' ? 'text-xs' : 'text-sm'
                }`}>
                {machine.dimensions}
              </div>
            </div>
            <div className="bg-[#000000]/50 p-3 rounded-lg border border-[#333333]">
              ...
              <div className={`text-[#F5F5F5] leading-tight ${variant === 'related' ? 'text-xs' : 'text-sm'
                }`}>
                {getBestForDisplay()}
              </div>
            </div>
        ...
        {/* Action Buttons */}
        {showActions && (
          <div className={`flex gap-3 mt-auto ${variant === 'compact' ? 'flex-col' : 'flex-col sm:flex-row'
            }`}>
            <Link
              href={`/vending-machines/${machine.id}`}
