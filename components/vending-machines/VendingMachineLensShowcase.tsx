...
import { getAllVendingMachines, MachineData } from '@/lib/data/vendingMachineData';

/**
 * VendingMachineShowcase
 * A component that displays vending machines with an interactive lens effect
 * for the home landing page
 */
export function VendingMachineLensShowcase() {
  // State for featured machines
  const [featuredMachines, setFeaturedMachines] = useState<MachineData[]>([]);
  // Current machine index to display
  const [currentIndex, setCurrentIndex] = useState(0);
  // State for lens hover effect
  const [hovering, setHovering] = useState(false);

  // Get machines on component mount
  useEffect(() => {
    // Get all machines and select featured ones
    const machines = getAllVendingMachines();
    // You could filter for specific machines or just use all
    setFeaturedMachines(machines);
  }, []);

  // Auto-rotate through machines
  useEffect(() => {
    if (featuredMachines.length === 0) return;
    
    const interval = setInterval(() => {
      if (!hovering) { // Only rotate if not being hovered
        setCurrentIndex(prevIndex => 
          prevIndex === featuredMachines.length - 1 ? 0 : prevIndex + 1
        );
      }
    }, 5000); // Change every 5 seconds
    
    return () => clearInterval(interval);
  }, [featuredMachines, hovering]);

  // No data yet or empty data
  if (featuredMachines.length === 0) {
    return <div className="h-80 flex items-center justify-center">Loading...</div>;
  }

  // Get the current machine to display
  const currentMachine = featuredMachines[currentIndex];

  return (
    <div className="my-8">
      <Lens zoomFactor={1.8} lensSize={200} hovering={hovering} setHovering={setHovering}>
        <div className="w-full relative rounded-3xl overflow-hidden max-w-md mx-auto bg-gradient-to-r from-[#1D2235] to-[#121318] p-8">
          ...
          <div className="relative z-10">
            ...
            <div className="relative w-full aspect-[4/3] rounded-2xl overflow-hidden">
              <Image
                src={currentMachine.images[0].src}
                alt={currentMachine.images[0].alt}
                fill
            ...
            >
              ...
              <p className="text-[#FD5A1E] text-sm font-medium mt-1">
                {currentMachine.model}
              </p>
              <p className="text-neutral-200 text-left mt-3 line-clamp-2">
                {currentMachine.shortDescription}
              </p>
              
              {/* Key Features */}
              <div className="mt-4">
                <h3 className="text-white text-sm font-medium mb-2">Key Features:</h3>
                <ul className="space-y-1">
                  {currentMachine.features.slice(0, 2).map((feature, index) => (
              ...
              <div className="mt-5">
                <Link 
                  ...
                  className="bg-[#FD5A1E]/90 hover:bg-[#FD5A1E] text-white px-4 py-2 rounded-lg text-sm font-medium transition-colors inline-flex items-center"
                >
                  View Details
              ...
              {featuredMachines.map((_, index) => (
                <button
                  ...
                  className={`w-2 h-2 rounded-full ${
                    index === currentIndex ? 'bg-[#FD5A1E]' : 'bg-white/30'
                  } transition-colors`}
                  aria-label={`View ${featuredMachines[index].name}`}
                />
              ))}
            </div>
          )}
...
// Background decorative elements - keeping the original styling
const Beams = () => {
  return (
    <svg
      width="380"
  ...
  return (
    ...
    >
      ...
      <defs>
        ...
        >
          <stop offset="0.214779" stopColor="#FD5A1E" />
